...
Texture* methodPercent(const char* str)
{
	...
	iSize size = iSizeMake(100 * SIZE_RATE, 40 * SIZE_RATE);
	g->init(size);
...
void loadLodingBar()
{
	...
	size = iSizeMake(devSize.width * 0.7f + 8 * SIZE_RATE, 30 * SIZE_RATE + 8 * SIZE_RATE);
	...
	size = iSizeMake(devSize.width * 0.7f, 30 * SIZE_RATE);
	...
	pop->closePoint = iPointMake(devSize.width * 0.15f - 4 * SIZE_RATE, devSize.height * 0.8f);
...
void drawLodingBarAfter(iPopup* pop, float dt, float rate)
{
	...
	iPoint p = pop->closePoint + 4 * SIZE_RATE;
