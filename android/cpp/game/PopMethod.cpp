...
{
	iGraphics* g = iGraphics::share();
	iSize size = iSizeMake(300 * SIZE_RATE, 70 * SIZE_RATE);
...
{
	iGraphics* g = iGraphics::share();
	iSize size = iSizeMake(300 * SIZE_RATE, 100 * SIZE_RATE);
...
Texture* methodTuto(const char* str)
{
	...
	g->drawString(size.width / 2, size.height / 2 - 60 * SIZE_RATE, VCENTER | HCENTER,
...
Texture* methodTutoScript(const char* str)
{
	...
	if (3 <= tutorial->chapter && tutorial->chapter <= 5)
		g->drawString(size.width / 2, 20 * SIZE_RATE, VCENTER | HCENTER,
...
Texture* methodInfo(const char* str)
{
	...
	{
		for (i = 0; i < 5; i++)
			g->drawString(size.width * 0.1f, 30 * SIZE_RATE + 40 * SIZE_RATE * i, TOP | LEFT, strP1[i]);
	}
	else if (p == 1)
	{
		for (i = 0; i < 6; i++)
			g->drawString(size.width * 0.1f, 30 * SIZE_RATE + 40 * SIZE_RATE * i, TOP | LEFT, strP2[i]);
	}
	else if (p == 2)
	{
		for (i = 0; i < 7; i++)
			g->drawString(i == 0 ? size.width * 0.1f : size.width * 0.2f, 30 * SIZE_RATE + 40 * SIZE_RATE * i, TOP | LEFT, strP3[i]);
	}
	else if (p == 3)
	{
		g->drawString(size.width * 0.1f, 30 * SIZE_RATE, TOP | LEFT, strP4[0]);
		g->drawString(size.width * 0.1f, 110 * SIZE_RATE, TOP | LEFT, strP4[1]);
		g->drawString(size.width * 0.1f, 150 * SIZE_RATE, TOP | LEFT, strP4[2]);
		g->drawString(size.width * 0.1f, 230 * SIZE_RATE, TOP | LEFT, strP4[3]);
		g->drawString(size.width * 0.1f, 270 * SIZE_RATE, TOP | LEFT, strP4[4]);
	}
	else if (p == 4)
	{
		for (i = 0; i < 6; i++)
			g->drawString(size.width * 0.1f, 30 * SIZE_RATE + 40 * SIZE_RATE * i, TOP | LEFT, strP5[i]);
	}
	g->drawString(size.width / 2, size.height - 20 * SIZE_RATE, VCENTER | HCENTER, "%d / %d", 1 + p, _pageInfo);
...
Texture* methodSet(const char* str)
{
	...
	setStringSize(25 * SIZE_RATE);
	setStringRGBA(RGB_ORANGE, 1.0f);

	for (i = 0; i < 4; i++)
	{
		p = iPointMake(size.width * 0.25f, size.height * 0.2f + 15 * SIZE_RATE + 50 * SIZE_RATE * i);
		switch (i)
		{
			case 0:
	...
	for (i = 0; i < 4; i++)
	{
		p = iPointMake(size.width * 0.625f, size.height * 0.2f + 15 * SIZE_RATE + 50 * SIZE_RATE * i);
		switch (i)
		{
			case 0:
...
{
	iGraphics* g = iGraphics::share();
	iSize size = iSizeMake(350 * SIZE_RATE, 40 * SIZE_RATE);
...
{
	iGraphics* g = iGraphics::share();
	iSize size = iSizeMake(150 * SIZE_RATE, 40 * SIZE_RATE);
...
Texture* methodOver(const char* str)
{
	...
	g->drawString(size.width / 2, h + 50 * SIZE_RATE, VCENTER | HCENTER, script[language].strWords.bestTime, bestTime);
	g->drawString(size.width / 2, h + 100 * SIZE_RATE, VCENTER | HCENTER, script[language].strWords.record, playTime);
...
Texture* methodHidden(const char* str)
{
	...
	setStringSize(40 * SIZE_RATE);
	setStringRGBA(RGB_BLUE, 1.0f);
	g->drawString(size.width / 2, 75 * SIZE_RATE, VCENTER | HCENTER, script[language].strWords.please);
