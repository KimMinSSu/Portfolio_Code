...
void loadPlayer()
{
	...
	p->delaySt = 0.0f;
	p->degree = 0.0f;
	p->position = iPointMake(devSize.width / 2, devSize.height + 100);//iPointCenter;
...
void drawPlayer(float dt, iPoint off)
{
	...
	if (p->paint(dt, off))
	{
		...
		if (procPlayerAppearDt < 6.5f)
		{
			...
			if (procPlayerAppearDt < _procPlayerAppearDt)
			{
				p->position = easeIn(procPlayerAppearDt / _procPlayerAppearDt,
									 iPointMake(devSize.width / 2, devSize.height + 100),
									 iPointCenter);
			}
			else
			{
		...
		p->walk = p->vec != iPointZero;
		p->position += p->vec * (p->speed * dt);
