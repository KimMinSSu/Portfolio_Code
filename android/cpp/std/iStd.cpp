...
void drawCircle(float x, float y, float radius)
{
    ...
    GLuint attr = glGetAttribLocation(id, "position");
    glEnableVertexAttribArray(attr);
    glVertexAttribPointer(attr, 4, GL_FLOAT, GL_FALSE, sizeof(0), (const void*)0);
    GLuint attrPosition = attr;
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        else
        {
            for (int y = 0; y < 32; y++)
            {
                for (int x = 0; x < 32; x++)
                {
                    uint8* c = &rgba[32 * 4 * y + 4 * x];
                    c[0] = c[1] = c[2] = 1;
                    c[3] = sqrtf((x - 15.5) * (x - 15.5) + (y - 15.5) * (y - 15.5)) > 15.5 ? 0 : 1;
                }
            }
        }
...
//{
//	if (dm == DrawModeShader)
//	{
//		drawLine = drawLine_shader;
//	}
//	else
//	{
//		drawLine = drawLine_image;
//	}
//}
...
Texture* createTexture(int width, int height, bool rgba32f)
{
    ...
    {
        //glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, width, height, 0, GL_RGBA, GL_HALF_APPLE, NULL);
...
const char* strFrag = "alpha";
void setFilterStr(const char* sf)
{
    strFrag = sf;
}
float iTime;
void setFilterTime(float time)
{
    iTime = time;
}
void drawImageFilter(Texture* tex, float x, float y, int anc,
                     float ix, float iy, float iw, float ih,
                     float rx, float ry,
                     int xyz, float degree, int reverse)
{
    ...
    GLuint id = vtx->useProgram("alpha", strFrag);
    VertexInfo vi[4];
    for (int i = 0; i < 4; i++)
    {
        memcpy(vi[i].position, &dstPoint[i], sizeof(iPoint));
