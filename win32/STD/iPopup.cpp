...
void iPopup::paint(float dt)
{
	...
	if (degree == 0.0f)
	{
		drawImage(t, p.x, p.y, TOP | LEFT,
			0, 0, t->width, t->height, rx, ry, 0, 0, REVERSE_HEIGHT);
	}
	else
	{
		p.x += (t->width / 2 * rx);
		...
		drawImage(t, p.x, p.y, VCENTER | HCENTER,
			0, 0, t->width, t->height, rx, ry, 0, degree, REVERSE_HEIGHT);
	}
...
void iPopup::show(bool show)
{
	...
	{
		if (bShow == false || stat != iPopupProc)
			return;
