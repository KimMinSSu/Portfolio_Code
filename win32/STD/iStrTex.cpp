...
void iStrTex::setString(const char* szFormat, ...)
{
	...
	else
	{
		...
		{
			rt = iGraphics::rectOfString(line[i]);
			g->drawString(-rt.origin.x, -rt.origin.y + slh * i, TOP | LEFT, line[i]);
		}
		//setRGBA(br, bg, bb, ba);
#endif
		//tex = g->getTexture();
...
void iStrTex::paint(float x, float y, int anc, const char* szFormat, ...)
{
	...
	setString(szText);
	drawImage(tex, x, y, anc, 0, 0, tex->width, tex->height, 1.0f, 1.0f, 0, 0, REVERSE_NONE);
}
...
void iStrTex::paint(iPoint p, int anc, const char* szFormat, ...)
{
	...
	setString(szText);
	drawImage(tex, p.x, p.y, anc, 0, 0, tex->width, tex->height, 1.0f, 1.0f, 0, 0, REVERSE_NONE);
}
...
void iStrTex::paint(float x, float y, int anc, float rx, float ry, const char* szFormat, ...)
{
	char szText[256];
	va_start_end(szText, szFormat);

	setString(szText);
	drawImage(tex, x, y, anc, 0, 0, tex->width, tex->height, rx, rx, 0, 0, REVERSE_NONE);
}
